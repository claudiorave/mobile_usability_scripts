...
function initCss(done) {
  ...
  link.onload = done;
  link.setAttribute(
    "href", "https://cdn.shopify.com/s/files/1/0070/3666/5911/files/superlemon_94807d87-f786-4e07-8978-255b03e8adb0.css?v=1659959651"
  );
...
function btnLoad() {
  ...
  function loadSpinWheelWidget(settings, current_device_type) {
    ...
    gameStatusTxt.innerHTML = "Special discount unlocked";

    var spinTheWheelBtn = document.createElement("button");
    spinTheWheelBtn.innerHTML = "Spin the Wheel";

    spinTheWheelBtn.onclick = onSpinWheelBtnClick;

    var gameRules = document.createElement("div");
    gameRules.classList = "spin-wheel-game-rules";
    gameRules.innerHTML = getSpinWheelGameRules();
  ...
  function getSpinWheelGameRules() {
    return `
  ...
  function onSpinWheelBtnClick(e) {
    ...
    setTimeout(() => {
      getSpinWheelResult(randomSpin);
    }, 2000);
  }

  function getSpinWheelResult(spin) {
    ...
    handleSpinWheelResult(list[result].code.length > 0 ? "WIN" : "LOOSE");
  }

  function renderNumberInputForm() {
    let winResult = spinWheelInitConfig.list[spinWheelInitConfig.result];
    jQuery("#spin-wheel-info").html(`
      <h1 class="game-status-txt">Yay!! you will get ${winResult.text}</h1>
      <p class="game-status-subtxt">We'll send the discount code and updates on your WhatsApp number</h1>
      `);
    ...
    spinWheelformSubmit.innerHTML = 'Get the Discount Code';
      ...
      if (phone && countryCode) {
        jQuery.ajax({
          ...
          success: function () {
            localStorage.setItem("opted_in_phone_v2", phone);
            localStorage.setItem("opted_in_country_code_v2", countryCode);
            handleSpinWheelResult("WIN_SUBMIT_FORM");
          },
          error: function (e) {
            if (e && e.status === 202) {
              // handle
  ...
  function handleSpinWheelResult(status) {
    switch (status) {
      // show mobile number input
      case 'WIN':
        renderNumberInputForm();
        break;
      // show loose message
      case 'LOOSE':
        jQuery("#spin-wheel-info").html(`
          <h1 class="game-status-txt">You almost win!</h1>
          <p class="game-status-subtxt">Better luck next time</h1>
          `);
      ...
      case 'WIN_SUBMIT_FORM':
        let winResult = spinWheelInitConfig.list[spinWheelInitConfig.result];
        jQuery("#spin-wheel-info").html(`
            <h1 class="game-status-txt">You win!</h1>
            <p class="game-status-subtxt">${winResult.text} <br/> Your Coupon code is: ${winResult.code}</h1>
            `);
        var copyCouponCodeBtn = document.createElement("button");
        copyCouponCodeBtn.innerHTML = 'Copy coupon code';
        ...
        jQuery("#spin-wheel-info").append(`<p class="spin-wheel-note-txt">We have also sent the coupon code on your number<p>`);
        break;
    }
  }
