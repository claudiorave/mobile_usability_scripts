...
window.xsellStats.track = function(){
  ...
  var referringPage = sessionStorage.getItem('setpage') || getCookie('setpage') || searchParams.get('xsell_source');
...
window.xsellStats.watchCrossSells = function(){
  var selectors = ['.cross-sell a:not([data-xs-link]):not([data-xs-quickview])', '.cross-sell input', '.cross-sell .cs_btn'];
